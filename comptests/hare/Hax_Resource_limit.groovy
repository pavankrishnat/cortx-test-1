...
'''
                ...
                stage("IO's") {
                    ...
                    python --version
                    deactivate
                    ssh root@ssc-vm-g4-rhev4-1587.colo.seagate.com "cat <<EOF >>/root/IO_tracker.log
IOs started
EOF"
                '''
                        }
                    ...
                    # . ./scripts/cicd_k8s_cortx_deploy/log_collecter.sh ${BUILD} ${SETUP_ENTRY} ${WORKSPACE} IOStability-PI7
                    # echo $LOG_PATH
                    deactivate
                    '''
                            } catch (err) {
                                build_stage = env.STAGE_NAME
                                sh label: '', script: '''
ssh root@ssc-vm-g4-rhev4-1587.colo.seagate.com "rm -rf /root/IO_tracker.log
'''
                                error "IO's Failed"
                            }
                        }
